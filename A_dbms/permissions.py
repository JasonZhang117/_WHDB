    ...
    for permission_key, permission_val in perm_dic.items():
        ...
        if per_url_name == current_url_name:  # matches current request url
            if per_method == request.method:  # matches request method
                '''if not  perm_args: #if no args defined in perm dic, 
                then set this request to passed perm
                ...
                for k, v in perm_kwargs.items():
                    ...
                    # request字典中有此参数
                    print("perm kwargs check:",
                          arg_val, type(arg_val),
                          v, type(v))
                    if arg_val == str(v):
...
def check_permission(func):
    def inner(*args, **kwargs):
        if not perm_check(*args, **kwargs):
            request = args[0]
            print('check_permission-->request',request)
