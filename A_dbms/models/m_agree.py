...
class Agrees(models.Model):  # 委托合同
    agree_num = models.CharField(verbose_name='_合同编号',
                                 max_length=32,
                                 unique=True)
    AGREE_NAME_LIST = (
        (1, '委托保证合同'),
    ...
    agree_name = models.IntegerField(verbose_name='合同种类',
                                     choices=AGREE_NAME_LIST)
    ...
    agree_start_date = models.DateField(verbose_name='起始日',
                                        null=True,
                                        blank=True)
    agree_due_date = models.DateField(verbose_name='到期日',
                                      null=True,
                                      blank=True)
    purpose = models.CharField(verbose_name='借款用途',
                               max_length=32,
                               null=True,
                               blank=True)
    acc_name = models.CharField(verbose_name='开户名',
                                max_length=32,
                                null=True,
                                blank=True)

    acc_num = models.CharField(verbose_name='账号',
                               max_length=32,
                               null=True,
                               blank=True)
    acc_bank = models.CharField(verbose_name='开户行',
                                max_length=64,
                                null=True,
                                blank=True)
    REPAY_METHOD_LIST = (
    ...
    agree_typ = models.IntegerField(verbose_name='合同种类',
    ...
    guarantee_typ = models.CharField(verbose_name='反担保种类数',
                                     max_length=6,
    ...
    agree_state = models.IntegerField(verbose_name='合同状态',
    ...
    supplementary = models.TextField(verbose_name='补充协议',
                                     null=True,
                                     blank=True)

    agree_buildor = models.ForeignKey(to='Employees',
                                      verbose_name="创建人",
                                      on_delete=models.PROTECT,
                                      related_name='agree_buildor_employee')
    agree_date = models.DateField(verbose_name='创建日期',
...
class LetterGuarantee(models.Model):
    ...
    letter_typ = models.IntegerField(verbose_name='保函类型',
                                     choices=LETTER_TYP_LIST)
    ...
    starting_date = models.DateField(verbose_name='起始日期',
                                     null=True,
                                     blank=True)
...
class Counters(models.Model):  # 反担保合同
    counter_num = models.CharField(verbose_name='_合同编号',
                                   max_length=32,
                                   unique=True)
    COUNTER_NAME_LIST = (
        (1, '保证反担保合同'),
    ...
    counter_name = models.IntegerField(verbose_name='合同种类',
                                       choices=COUNTER_NAME_LIST,
                                       null=True,
                                       blank=True)
    ...
    COUNTER_TYP_LIST = ((1, '企业担保'), (2, '个人保证'), (11, '房产抵押'), (12, '土地抵押'),
                        (13, '动产抵押'), (14, '在建工程抵押'), (15, '车辆抵押'),
                        (31, '应收质押'), (32, '股权质押'), (33, '票据质押'), (34, '动产质押'),
                        (41, '其他权利质押'), (51, '股权预售'), (52, '房产预售'),
                        (53, '土地预售'), (59, '其他预售'))
    counter_typ = models.IntegerField(verbose_name='合同类型',
    ...
    counter_other = models.TextField(verbose_name='其他约定',
                                     null=True,
                                     blank=True)
    COUNTER_STATE_LIST = [(11, '未签订'), (21, '已签订'), (31, '作废')]
    counter_state = models.IntegerField(verbose_name='签订状态',
                                        choices=COUNTER_STATE_LIST,
                                        default=11)
    counter_sign_date = models.DateField(verbose_name='签订日期',
                                         null=True,
                                         blank=True)
    counter_remark = models.TextField(verbose_name='签订备注',
                                      null=True,
                                      blank=True)
    counter_view = models.TextField(verbose_name='合同预览', null=True, blank=True)

    counter_buildor = models.ForeignKey(to='Employees',
                                        verbose_name="创建者",
                                        default=1,
                                        on_delete=models.PROTECT,
                                        related_name='counteror_employee')
    counter_date = models.DateField(verbose_name='创建日期',
                                    default=datetime.date.today)
...
class CountersAssure(models.Model):  # 保证反担保合同
    ...
    counter_assure_buildor = models.ForeignKey(
        to='Employees',
        verbose_name="创建者",
        on_delete=models.PROTECT,
        related_name='counter_assureor_employee')
    counter_assure_date = models.DateField(verbose_name='创建日期',
                                           default=datetime.date.today)
...
class CountersWarrants(models.Model):  # 房产抵押反担保合同
    ...
    counter_warrant_buildor = models.ForeignKey(
        to='Employees',
        verbose_name="创建者",
        on_delete=models.PROTECT,
        related_name='counter_warrantor_employee')
    counter_warrant_date = models.DateField(verbose_name='创建日期',
                                            default=datetime.date.today)
...
class ResultState(models.Model):  #
    ...
    result_typ = models.IntegerField(verbose_name='决议类型',
                                     choices=RESULT_TYP_LIST)
    result_detail = models.TextField(verbose_name='决议声明内容',
                                     blank=True,
                                     null=True)
    resultor = models.ForeignKey(to='Employees',
                                 verbose_name="创建者",
                                 on_delete=models.PROTECT,
                                 related_name='resultor_employee')
    result_date = models.DateField(verbose_name='创建日期',
                                   default=datetime.date.today)
