...
class Agrees(models.Model):  # 委托合同
    agree_num = models.CharField(
        verbose_name='_合同编号', max_length=32, unique=True)
    num_prefix = models.CharField(
        verbose_name='_编号前缀', max_length=32)
    '''((1, '待反馈'), (2, '已反馈'), (3, '待上会'),
     (4, '已上会'), (5, '已签批'), (6, '已注销'))'''
    ...
    branch = models.ForeignKey(to='Branches',
                               verbose_name="放款银行",
                               on_delete=models.PROTECT,
    ...
    agree_state = models.IntegerField(
        verbose_name='_合同状态', choices=AGREE_STATE_LIST, default=1)
...
class Counters(models.Model):  # 反担保合同
    ...
    COUNTER_TYP_LIST = (
        (1, '企业担保'), (2, '个人担保'), (3, '房产抵押'), (4, '土地抵押'),
        (5, '应收质押'), (6, '动产抵押'), (7, '车辆抵押'))
    counter_typ = models.IntegerField(
        verbose_name='合同类型', choices=COUNTER_TYP_LIST)
    COUNTER_STATE_LIST = ((1, '未签订'), (2, '已签订'), (3, '已注销'))
    counter_state = models.IntegerField(
        verbose_name='_签订情况', choices=COUNTER_STATE_LIST, default=1)
...
class CountersHouse(models.Model):  # 房产抵押反担保合同
    ...
    house = models.ManyToManyField(to='Houses',
                                   verbose_name="抵押物",
    ...
    def __str__(self):
        return "%s_%s" % (self.counter.counter_num, self.house)
