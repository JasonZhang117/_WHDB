...
class AgreeAddForm(dform.ModelForm):
    ...
    class Meta:
        model = models.Agrees
        fields = [
            'lending', 'agree_typ', 'guarantee_typ', 'agree_copies',
            'agree_amount', 'amount_limit', 'agree_term', 'agree_term_typ',
            'other'
        ]
        widgets = {
            'lending':
            ...
            'other':
            dform.Textarea(attrs={
                'class': 'form-control',
                'rows': '2',
                'placeholder': '其他合同约定事项'
            }),
        }

    def __init__(self, *args, **kwargs):
        super(AgreeAddForm, self).__init__(*args, **kwargs)
        self.fields['branch'].choices = models.Branches.objects.filter(
            branch_state=1).values_list('id', 'name').order_by('name')
...
class AgreeEditForm(dform.ModelForm):
    class Meta:
        model = models.Agrees
        fields = [
            'branch', 'agree_typ', 'agree_amount', 'amount_limit',
            'agree_rate', 'investigation_fee', 'agree_term', 'agree_term_typ',
        ...
        widgets = {
            ...
            dform.TextInput(
                attrs={
                    'class': 'form-control',
                    'placeholder': '如为单项合同输入纯数字,委托担保合同单位：百分之，小贷合同单位：千分之'
                }),
            'investigation_fee':
