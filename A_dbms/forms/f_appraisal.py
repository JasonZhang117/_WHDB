...
class ArticlesSignForm(dform.Form):  # 项目签批
    summary_num = fields.CharField(
        label='纪要编号', label_suffix="：",
        widget=widgets.TextInput(
            attrs={'class': 'form-control', 'placeholder': '纪要编号'}))
    SIGN_TYPE_LIST = models.Articles.SIGN_TYPE_LIST
    sign_type = fields.IntegerField(
        label='签批结论', label_suffix="：", initial=1,
        widget=widgets.Select(
            choices=SIGN_TYPE_LIST, attrs={'class': 'form-control'}))
    renewal = fields.FloatField(
        label='本次续贷', label_suffix="：",
        widget=widgets.NumberInput(
            attrs={'class': 'form-control', 'placeholder': '本次续贷额度（元）'}))
    augment = fields.FloatField(
        label='本次新增', label_suffix="：",
        widget=widgets.NumberInput(
            attrs={'class': 'form-control', 'placeholder': '本次新增额度（元）'}))
    credit_amount = fields.FloatField(
        label='客户总授信', label_suffix="：",
        widget=widgets.NumberInput(
            attrs={'class': 'form-control', 'placeholder': '客户总授信额度（元）'}))
    sign_detail = fields.CharField(
        label='签批详情', label_suffix="：",
        widget=widgets.Textarea(
            attrs={'class': 'form-control', 'style': 'height:100xp ',
                   'placeholder': '签批详情'}))
...
class LendinReceivableForm(dform.Form):
    sure_receivable = fields.TypedMultipleChoiceField(
        ...
        widget=widgets.SelectMultiple(
            attrs={'class': 'form-control', 'placeholder': '应收账款'}))
...
class LendinStockForm(dform.Form):
    sure_stock = fields.TypedMultipleChoiceField(
        ...
        widget=widgets.SelectMultiple(
            attrs={'class': 'form-control', 'placeholder': '股权'}))
