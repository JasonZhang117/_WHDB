...
def review(request, *args, **kwargs):  # 保后列表
    ...
    if search_key:
        search_fields = [
            'name', 'short_name', 'district__name', 'idustry__name',
            'managementor__name', 'controler__name'
        ]
        q = Q()
        q.connector = 'OR'
...
@login_required
@authority
def article_book(request, *args, **kwargs):  # 风险台账
    current_url_name = resolve(request.path).url_name  # 获取当前URL_NAME
    authority_list = request.session.get('authority_list')  # 获取当前用户的所有权限
    menu_result = MenuHelper(request).menu_data_list()
    job_list = request.session.get('job_list')  # 获取当前用户的所有角色
    ...
    return render(request, 'dbms/review/book.html', locals())
