        ...
        if counter_obj.counter_state in [31]:
            ...
            try:
                counter_list.update(
                    ...
                    counter_sign_date=counter_sign_cleaned[
                        'counter_sign_date'],
    ...
    if form_ascertain_add.is_valid():
        ...
        if agree_state_y in [21, 25, 31, 51]:
            try:
                ...
                agree_list.update(agree_state=agree_state,
                                  ascertain_date=today_str,
                                  agree_remark=agree_remark)
        ...
        if form_notify_add.is_valid():
            ...
            if time_limit > agree_term:
                ...
                response['message'] = '通知期限（%s个月）大于合同期限（%s个月），放款通知添加失败！' % (
                    time_limit, agree_term)
            else:
                notify_money = form_notify_cleaned['notify_money']  # 通知金额
                weighting_money = round(
                    (time_limit / agree_term) * notify_money, 2)  # 时间加权通知金额
                weighting_amount = models.Notify.objects.filter(
                    agree=agree_obj).aggregate(Sum('weighting'))
                weighting_money_sum = weighting_amount[
                    'weighting__sum']  # 时间加权通知金额合计（已有）
                ...
                if amount > amount_limit:
                    response['status'] = False
                    response['message'] = '加权放款通知金额合计（%s）大于合同放款限额（%s）' % (
                        amount, amount_limit)
                elif agree_balance_notify > amount_limit:
                    response['status'] = False
                    response['message'] = '担保余额与通知金额合计（%s）大于合同放款限额（%s）' % (
                        agree_balance_notify, amount_limit)
                else:
                    try:
                        with transaction.atomic():
                            notify_obj = models.Notify.objects.create(
                                ...
                                weighting=weighting_money,
                                notify_date=form_notify_cleaned['notify_date'],
                                contracts_lease=form_notify_cleaned[
                                    'contracts_lease'],
                                contract_guaranty=form_notify_cleaned[
                                    'contract_guaranty'],
                                remark=form_notify_cleaned['remark'],
                            ...
                            article_list.update(article_notify_sum=round(
                                article_notify_amount, 2))
        ...
        try:
            with transaction.atomic():
                models.Agrees.objects.filter(notify_agree=notify_obj).update(
                    agree_notify_sum=F('agree_notify_sum') -
                    notify_obj.notify_money)
                models.Articles.objects.filter(
                    lending_summary__agree_lending__notify_agree=notify_obj
                ).update(article_notify_sum=F('article_notify_sum') -
                         notify_obj.notify_money)  # 项目
                notify_obj.delete()
...
def provide_add_ajax(request):
    ...
    if form_provide_add.is_valid():
        ...
        if amount > notify_obj.notify_money:
            response['status'] = False
            response['message'] = '放款金额合计（%s）大于放款通知金额（%s）' % (
                amount, notify_obj.notify_money)
        else:
            try:
                with transaction.atomic():
                    ...
                    notify_list.update(notify_provide_sum=amount,
                                       notify_balance=round(
                                           notify_provide_balance,
                                           2))  # 放款通知，更新放款总额
                    '''agree_provide_sum，更新合同放款信息'''
                    agree_list = models.Agrees.objects.filter(
                        notify_agree=notify_obj)  # 合同
                    agree_obj = agree_list.first()
                    agree_provide_amount = models.Provides.objects.filter(
                        notify__agree=agree_obj).aggregate(Sum(
                            'provide_money'))['provide_money__sum']  # 合同项下放款合计
                    agree_provide_balance = models.Provides.objects.filter(
                        notify__agree=agree_obj).aggregate(
                            Sum('provide_balance'))[
                                'provide_balance__sum']  # 合同项下在保余额合计
                    agree_list.update(
                        agree_provide_sum=round(agree_provide_amount, 2),
                        agree_balance=round(agree_provide_balance,
                                            2))  # 合同，更新放款总额
                    ...
                    lending_weight_amount = models.Notify.objects.filter(
                        agree__lending=lending_obj).aggregate(
                            Sum('weighting'))['weighting__sum']
                    if round(lending_weight_amount,
                             2) == round(lending_obj.order_amount, 2):
                        lending_list.update(
                            lending_provide_sum=round(lending_provide_amount,
                                                      2),
                    ...
                    article_weighting_amount = models.Notify.objects.filter(
                        agree__lending__summary=article_obj).aggregate(
                            Sum('weighting'))['weighting__sum']
                    if round(article_weighting_amount,
                             2) == round(article_obj.amount, 2):
                        article_list.update(
                            article_provide_sum=round(article_provide_amount,
                                                      2),
                            article_state=52,
                    ...
                    custom_list = models.Customes.objects.filter(
                        article_custom=article_obj)
                    custom_obj = custom_list.first()
                    branch_list = models.Branches.objects.filter(
                        agree_branch=agree_obj)
                    ...
                    custom_provide_balance = models.Provides.objects.filter(
                        ...
                        provide_typ=provide_typ).aggregate(
                            Sum('provide_balance'))[
                                'provide_balance__sum']  # 客户及放款品种项下，在保余额
                    branch_provide_balance = models.Provides.objects.filter(
                        notify__agree__branch=branch_obj,
                        provide_typ=provide_typ).aggregate(
                            Sum('provide_balance'))[
                                'provide_balance__sum']  # 放款银行及放款品种项下，在保余额
                    ...
                    cooperator_provide_balance = models.Provides.objects.filter(
                        ...
                        provide_typ=provide_typ).aggregate(
                            Sum('provide_balance'))[
                                'provide_balance__sum']  # 授信银行及放款品种项下，在保余额
                    '''PROVIDE_TYP_LIST = [(1, '流贷'), (11, '承兑'), (21, '保函'), (31, '委贷'), (41, '小贷')]'''
                    custom_list.update(
                        provide_date=form_provide_cleaned['provide_date'],
                    )  # 客户，更新流贷余额
                    if provide_typ == 1:  # (1, '流贷')
                        custom_list.update(custom_flow=round(
                            custom_provide_balance, 2))  # 客户，更新流贷余额
                        branch_list.update(branch_flow=round(
                            branch_provide_balance, 2))  # 放款银行，更新流贷余额
                        cooperator_list.update(cooperator_flow=round(
                            cooperator_provide_balance, 2))
                    elif provide_typ == 11:  # (11, '承兑')
                        custom_list.update(custom_accept=round(
                            custom_provide_balance, 2))  # 客户，更新承兑余额
                        branch_list.update(branch_accept=round(
                            branch_provide_balance, 2))  # 放款银行，更新承兑余额
                        cooperator_list.update(cooperator_accept=round(
                            cooperator_provide_balance, 2))
                    elif provide_typ == 21:  # (21, '保函')
                        custom_list.update(custom_back=round(
                            custom_provide_balance, 2), )  # 客户，更新保函余额
                        branch_list.update(branch_back=round(
                            branch_provide_balance, 2))  # 放款银行，更新保函余额
                        cooperator_list.update(cooperator_back=round(
                            cooperator_provide_balance, 2))
                    elif provide_typ == 31:  # (31, '委贷')
                        custom_list.update(entrusted_loan=round(
                            custom_provide_balance, 2), )  # 客户，更新委贷余额
                        branch_list.update(entrusted_loan=round(
                            branch_provide_balance, 2))  # 放款银行，更新委贷余额
                        cooperator_list.update(entrusted_loan=round(
                            cooperator_provide_balance, 2))
                    elif provide_typ == 41:  # (41, '小贷')
                        custom_list.update(petty_loan=round(
                            custom_provide_balance, 2))  # 客户，更新委贷余额
                        branch_list.update(petty_loan=round(
                            branch_provide_balance, 2))  # 放款银行，更新委贷余额
                        cooperator_list.update(
                            petty_loan=round(cooperator_provide_balance, 2))
                    '''更新客户、放款银行、授信银行在保总额'''

                    custom_provide_balance_all = models.Provides.objects.filter(
                        notify__agree__lending__summary__custom=custom_obj
                    ).aggregate(Sum('provide_balance'))[
                        'provide_balance__sum']  # 客户项下，在保余额
                    v_radio = radio(custom_provide_balance_all,
                                    custom_obj.g_value)
                    custom_list.update(amount=round(custom_provide_balance_all,
                                                    2),
                                       v_radio=v_radio)
                    ...
                    branch_list.update(
                        amount=round(branch_provide_balance_all, 2))
...
def provide_state_change_ajax(request):  #
    ...
    if form_change_provide_state.is_valid():
        ...
        try:
            with transaction.atomic():
                provide_list.update(
                    provide_status=provide_state_cleaned['provide_status'], )
        ...
        try:
            with transaction.atomic():
                default = {
                    ...
                    provide_obj,
                    'extension_amount':
                    str(extension_cleaned['extension_amount']),
                    'extension_date':
                    str(extension_cleaned['extension_date']),
                    'extension_due_date':
                    str(extension_cleaned['extension_due_date']),
                    'extensionor':
                    request.user
                }
                extension_obj, created = models.Extension.objects.update_or_create(
                    ...
                    defaults=default)
                provide_list.update(due_date=str(
                    extension_cleaned['extension_due_date']),
                                    provide_status=15)
    ...
    try:
        with transaction.atomic():
            ...
            if notify_provide_amount:
                notify_list.update(
                    notify_provide_sum=round(notify_provide_amount, 2),
                    notify_balance=round(notify_provide_balance,
                                         2))  # 放款通知，更新放款总额
            else:
                notify_list.update(notify_provide_sum=0,
                                   notify_balance=0)  # 放款通知，更新放款总额
            '''agree_provide_sum，更新合同放款信息'''
            agree_list = models.Agrees.objects.filter(
                notify_agree=notify_obj)  # 合同
            agree_obj = agree_list.first()
            agree_provide_amount = models.Provides.objects.filter(
                notify__agree=agree_obj).aggregate(
                    Sum('provide_money'))['provide_money__sum']  # 合同项下放款合计
            agree_provide_balance = models.Provides.objects.filter(
                notify__agree=agree_obj).aggregate(Sum('provide_balance'))[
                    'provide_balance__sum']  # 合同项下在保余额合计
            if agree_provide_amount:
                agree_list.update(agree_provide_sum=round(
                    agree_provide_amount, 2),
                                  agree_balance=round(agree_provide_balance,
                                                      2))  # 合同，更新放款总额
            else:
                agree_list.update(agree_provide_sum=0,
                                  agree_balance=0)  # 合同，更新放款总额
            ...
            if lending_provide_amount:
                lending_list.update(
                    lending_provide_sum=round(lending_provide_amount, 2),
                ...
                '''LENDING_STATE = [(3, '待上会'), (4, '已上会'), (5, '已签批'),
                                                             (51, '已放款'), (52, '已放完'), (55, '已解保'),
                                                              (61, '待变更'),(99, '已注销')]'''
                lending_list.update(lending_provide_sum=0,
                                    lending_balance=0,
                                    lending_state=5)  # 放款次序，更新放款总额
            '''article_provide_sum，更新项目放款信息'''
            article_list = models.Articles.objects.filter(
                lending_summary=lending_obj)  # 项目
            article_obj = article_list.first()
            ...
            if article_provide_amount:
                ...
                article_list.update(
                    article_provide_sum=round(article_provide_amount, 2),
                    article_balance=round(article_provide_balance, 2),
                    article_state=51)  # 项目，更新放款总额
            else:
                article_list.update(article_provide_sum=0,
                                    article_balance=0,
                                    article_state=5)  # 项目，更新放款总额
            '''更新客户余额信息,custom_flow,custom_accept,custom_back'''
            '''更新银行余额信息,branch_flow,branch_accept,branch_back'''
            custom_list = models.Customes.objects.filter(
                article_custom=article_obj)
            ...
            custom_provide_balance = models.Provides.objects.filter(
                notify__agree__lending__summary__custom=custom_obj,
                provide_typ=provide_typ).aggregate(Sum('provide_balance'))[
                    'provide_balance__sum']  # 客户及放款品种项下，在保余额
            if custom_provide_balance:
                if provide_typ == 1:
                    custom_list.update(
                        custom_flow=custom_provide_balance)  # 客户，更新流贷余额
                elif provide_typ == 11:
                    custom_list.update(
                        custom_accept=custom_provide_balance)  # 客户，更新承兑余额
                elif provide_typ == 21:
                    custom_list.update(
                        custom_back=custom_provide_balance)  # 客户，更新保函余额
                elif provide_typ == 31:
                    custom_list.update(
                        entrusted_loan=custom_provide_balance)  # 客户，更新委贷余额
                elif provide_typ == 41:
                    custom_list.update(
                        petty_loan=custom_provide_balance)  # 客户，更新小贷余额
            else:
                if provide_typ == 1:
            ...
            if branch_provide_balance:
                if provide_typ == 1:
                    branch_list.update(
                        branch_flow=branch_provide_balance)  # 放款银行，更新流贷余额
                elif provide_typ == 11:
                    branch_list.update(
                        branch_accept=branch_provide_balance)  # 放款银行，更新承兑余额
                elif provide_typ == 21:
                    branch_list.update(
                        branch_back=branch_provide_balance)  # 放款银行，更新保函余额
                elif provide_typ == 31:
                    branch_list.update(
                        entrusted_loan=branch_provide_balance)  # 放款银行，更新委贷余额
                elif provide_typ == 41:
                    branch_list.update(
                        petty_loan=branch_provide_balance)  # 放款银行，更新小贷余额
            else:
                if provide_typ == 1:
...
def repayment_add_ajax(request):
    ...
    if form_repayment_add.is_valid():
        ...
        repayment_amount = models.Repayments.objects.filter(
            provide=provide_obj).aggregate(Sum('repayment_money'))
        ...
        if amount > round(provide_obj.provide_money, 2):
            response['status'] = False
            response['message'] = '还款金额合计（%s）大于放款金额（%s）' % (
                amount, provide_obj.provide_money)
        else:
            try:
                with transaction.atomic():
                    repayment_obj = models.Repayments.objects.create(
                        ...
                        repayment_int=repayment_cleaned['repayment_int'],
                        repayment_pen=repayment_cleaned['repayment_pen'],
...
def change_agree_state_ajax(request):  #
    ...
    if form_change_agree_state.is_valid():
        ...
        try:
            with transaction.atomic():
                agree_list.update(
                    agree_state=change_agree_cleaned['agree_state'], )
...
def repayment_del_ajax(request):  # 删除还款信息ajax
    ...
    if provide_obj.provide_status == 1:
        try:
            with transaction.atomic():
                ...
                provide_repayment_amount = models.Repayments.objects.filter(
                    provide=provide_obj).aggregate(Sum('repayment_money'))[
                        'repayment_money__sum']  # 放款项下还款合计
                if provide_repayment_amount:
                    provide_balance = round(
                        provide_obj.provide_money - provide_repayment_amount,
                        2)
                    provide_list.update(
                        provide_repayment_sum=round(provide_repayment_amount,
                                                    2),
                        provide_balance=provide_balance)  # 放款，更新还款总额
                else:
                    provide_balance = round(provide_obj.provide_money, 2)
                    provide_list.update(
                        provide_repayment_sum=0,
                        provide_balance=provide_balance)  # 放款，更新还款总额
                ...
                notify_repayment_amount = models.Repayments.objects.filter(
                    provide__notify=notify_obj).aggregate(
                        Sum('repayment_money'))[
                            'repayment_money__sum']  # 通知项下还款合计
                notify_provide_balance = models.Provides.objects.filter(
                    notify=notify_obj).aggregate(
                        Sum('provide_balance'))['provide_balance__sum']
                if notify_repayment_amount:
                    notify_list.update(
                        notify_repayment_sum=round(notify_repayment_amount, 2),
                        notify_balance=round(notify_provide_balance,
                                             2))  # 放款通知，更新还款总额
                else:
                ...
                if agree_repayment_amount:
                    ...
                    agree_list.update(agree_repayment_sum=0,
                                      agree_state=31,
                                      agree_balance=round(
                                          agree_provide_balance,
                                          2))  # 合同，更新还款总额
                ...
                if lending_repayment_amount:
                    lending_list.update(
                        lending_repayment_sum=round(lending_repayment_amount,
                                                    2),
                    ...
                    lending_list.update(lending_repayment_sum=0,
                                        lending_state=lending_state_n,
                                        lending_balance=round(
                                            lending_provide_balance,
                                            2))  # 放款次序，更新还款总额
                ...
                article_repayment_amount = models.Repayments.objects.filter(
                    provide__notify__agree__lending__summary=article_obj
                ).aggregate(Sum('repayment_money'))['repayment_money__sum']
                article_provide_balance = models.Provides.objects.filter(
                    notify__agree__lending__summary=article_obj).aggregate(
                        Sum('provide_balance'))['provide_balance__sum']
                '''ARTICLE_STATE_LIST = ((1, '待反馈'), (2, '已反馈'), (3, '待上会'), (4, '已上会'), (5, '已签批'),
                ...
                if article_repayment_amount:
                    article_list.update(
                        article_repayment_sum=round(article_repayment_amount,
                                                    2),
                        article_state=51,
                        article_balance=round(article_provide_balance,
                                              2))  # 项目，更新还款总额
                else:
                    article_list.update(article_repayment_sum=0,
                                        article_state=51,
                                        article_balance=round(
                                            article_provide_balance,
                                            2))  # 项目，更新还款总额
                '''更新客户余额信息,custom_flow,custom_accept,custom_back'''
                '''更新银行余额信息,branch_flow,branch_accept,branch_back'''
                custom_list = models.Customes.objects.filter(
        ...
        response['status'] = False
        response[
            'message'] = '该笔放款状态为：%s，还款信息删除失败！！！' % provide_obj.provide_status
    result = json.dumps(response, ensure_ascii=False)
    return HttpResponse(result)
...
def track_plan_ajax(request):
    ...
    if form_track_plan.is_valid():
        ...
        ttt = models.Track.objects.filter(provide=provide_obj,
                                          plan_date=plan_date,
                                          track_typ=track_typ)
        if ttt:
            response['status'] = False
    ...
    if form_track_ex_add.is_valid() and form_track_add.is_valid():
        ...
        try:
            with transaction.atomic():
                ...
                track_list.update(
                    term_pried=ex_pried_amont,
                    term_inted=ex_inted_amont,
                    term_pened=ex_pened_amont,
                    track_state=track_ed['track_state'])  # 更新跟踪计划
                repayment_obj = models.Repayments.objects.create(
                    provide=provide_obj,
...
def track_state_change_ajax(request):  #计划状态ajax
    ...
    if form_track_state_change.is_valid():
        ...
        try:
            with transaction.atomic():
                track_list.update(
                    track_state=track_state_cleaned['track_state'], )
