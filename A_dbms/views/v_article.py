...
from _WHDB.views import (MenuHelper, authority, article_right,
                         article_list_screen, UNX, UND, convert_str, amount_s,
                         amount_y, un_dex, convert_num, convert)
...
def article_scan(request, article_id):  # 项目预览
    ...
    form_date = {
        ...
        'product_id': article_obj.product.id,
        'renewal': article_obj.renewal,
        'process_id': article_obj.process.id,
    ...
    if article_obj.article_state in [1, 2, 3, 4]:
        ...
        form_date = {
            ...
            'renewal': article_obj.renewal,
            'augment': article_obj.augment,
            'credit_amount': article_obj.custom.credit_amount,
            'rcd_opinion': article_obj.rcd_opinion,
            'convenor_opinion': article_obj.convenor_opinion,
    ...
    form_opinion = forms.FormOpinion(
        initial={
            'article_repay_method': article_obj.article_repay_method,
            'opinion': article_obj.opinion
        })  # 添加放款次序form
...
def article_scan_lending(request, article_id, lending_id):  # 项目预览
    ...
    from_jk_data = {
        'agree_start_date': str(today_str),
        'agree_due_date': str(date_th_later),
    ...
    form_lendingcustoms_c_add = models.Customes.objects.exclude(
        id=article_obj.custom.id).filter(genre=1).values_list(
            'id', 'name')  # 除项目客户外的企业
    form_lendingcustoms_p_add = models.Customes.objects.exclude(
        id=article_obj.custom.id).filter(genre=2).values_list(
            'id', 'name')  # 除项目客户外的个人
    ...
    form_lendingconstruct_add = forms.LendingConstructForm(
    )  # 添加（反）担保-在建工程form
